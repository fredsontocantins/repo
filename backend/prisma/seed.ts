import {
  PrismaClient, UserRole, VolunteerStatus, CampaignStatus,
  DonationType, DonationStatus, EventStatus, MemberRole, CertificateType
} from '@prisma/client'
...
async function main() {
  ...
  // ══════════════════════════════════════════════════════════
  const membersData = [
    { nome: 'Patrícia Drummond',  email: 'patricia@vu.org.br',  cargo: 'Diretora Executiva',             role: MemberRole.DIRECTOR,    departamento: 'Gestão',        telefone: '(11) 98001-0001', bio: 'Fundadora da organização, 15 anos de experiência em gestão de ONGs.' },
    { nome: 'Rafael Monteiro',    email: 'rafael@vu.org.br',    cargo: 'Gerente de Operações',           role: MemberRole.MANAGER,     departamento: 'Operações',     telefone: '(11) 98001-0002', bio: 'Responsável por toda a logística dos projetos e eventos.' },
    { nome: 'Camila Ferreira',    email: 'camila@vu.org.br',    cargo: 'Coordenadora de Voluntariado',   role: MemberRole.COORDINATOR, departamento: 'Voluntariado',  telefone: '(11) 98001-0003', bio: 'Conecta voluntários às campanhas certas desde 2020.' },
    { nome: 'Diego Nascimento',   email: 'diego@vu.org.br',     cargo: 'Analista de Captação',           role: MemberRole.ANALYST,     departamento: 'Captação',      telefone: '(11) 98001-0004' },
    { nome: 'Juliana Castro',     email: 'juliana@vu.org.br',   cargo: 'Analista de Comunicação',        role: MemberRole.ANALYST,     departamento: 'Comunicação',   telefone: '(11) 98001-0005' },
    { nome: 'Marcos Pinheiro',    email: 'marcos@vu.org.br',    cargo: 'Analista Financeiro',            role: MemberRole.ANALYST,     departamento: 'Financeiro',    telefone: '(11) 98001-0006' },
    { nome: 'Thiago Cardoso',     email: 'thiago@vu.org.br',    cargo: 'Estagiário de TI',               role: MemberRole.INTERN,      departamento: 'TI' },
    { nome: 'Letícia Rocha',      email: 'leticia@vu.org.br',   cargo: 'Estagiária de Comunicação',      role: MemberRole.INTERN,      departamento: 'Comunicação' },
  ]
  ...
  for (const b of badgeData) {
    const badge = await prisma.badge.create({ data: b }).catch(async () =>
      prisma.badge.findFirst({ where: { nome: b.nome } })
    )
    badges.push(badge)
  }
  ...
  for (const v of vData) {
    const vol = await prisma.volunteer.create({
      data: { ...v, status: VolunteerStatus.ACTIVE, publicProfile: true, organizationId: org.id },
  ...
  await prisma.volunteer.create({
    data: { nome: 'Sandra Melo', email: 'sandra.melo@email.com', profissao: 'Fisioterapeuta', habilidades: ['fisioterapia', 'reabilitação'], pontos: 0, horasContribuidas: 0, status: VolunteerStatus.PENDING, publicProfile: false, organizationId: org.id },
  }).catch(() => {})
  await prisma.volunteer.create({
    data: { nome: 'Felipe Nunes', email: 'felipe.nunes@email.com', profissao: 'Designer Gráfico', habilidades: ['design', 'criação visual'], pontos: 0, horasContribuidas: 0, status: VolunteerStatus.PENDING, publicProfile: false, organizationId: org.id },
  }).catch(() => {})

  console.log('✅ Voluntários:', volunteers.length + 2)

  // Badges para voluntários
  const badgeAssignments = [
    { vi: 0, bis: [0, 1, 2, 4] },  // João — todos os principais
    { vi: 3, bis: [0, 1, 2, 4, 5] }, // Fernanda — + saúde
    { vi: 8, bis: [0, 1, 2, 5] },  // Marcelo — saúde
    { vi: 1, bis: [0, 1, 7] },     // Ana — educação
    { vi: 7, bis: [0, 1, 3] },     // Larissa — doadora
    { vi: 9, bis: [0, 1, 2] },     // Tatiane
    { vi: 4, bis: [0, 3] },        // Roberto
    { vi: 2, bis: [0, 6] },        // Carlos — verde
  ]
  for (const a of badgeAssignments) {
    if (!volunteers[a.vi]) continue
    for (const bi of a.bis) {
      if (!badges[bi]) continue
  ...
  // Associar voluntários às campanhas
  const campVolAssoc = [
    { ci: 0, vis: [0, 7, 8, 4, 9] },
    { ci: 1, vis: [1, 6, 11] },
    { ci: 2, vis: [0, 8, 3, 5, 9] },
  ...
  for (const a of campVolAssoc) {
    if (!campaigns[a.ci]) continue
    for (const vi of a.vis) {
      if (!volunteers[vi]) continue
  ...
  // ══════════════════════════════════════════════════════════
  const donationsData = [
    { tipo: DonationType.MONETARY, valor: 5000,  doadorNome: 'Grupo Empresarial Horizonte',    doadorEmail: 'financeiro@horizonte.com.br',  status: DonationStatus.CONFIRMED, campaignId: campaigns[0].id, mensagem: 'Com muito carinho para a causa da alimentação.' },
    { tipo: DonationType.MONETARY, valor: 2500,  doadorNome: 'Adriana Meirelles',              doadorEmail: 'adriana@email.com',             status: DonationStatus.CONFIRMED, campaignId: campaigns[0].id },
    { tipo: DonationType.MONETARY, valor: 1000,  doadorNome: 'João Carlos Pereira',            doadorEmail: 'jcp@email.com',                 status: DonationStatus.CONFIRMED, campaignId: campaigns[0].id },
    { tipo: DonationType.FOOD,     descricao: '300 kg de arroz, feijão, macarrão e óleo',     doadorNome: 'Supermercado Bem-Estar',                                          status: DonationStatus.CONFIRMED, campaignId: campaigns[0].id },
    { tipo: DonationType.FOOD,     descricao: '150 cestas básicas completas',                  doadorNome: 'Associação Comercial do Bairro',                                  status: DonationStatus.CONFIRMED, campaignId: campaigns[0].id },
    { tipo: DonationType.MONETARY, valor: 8000,  doadorNome: 'Fundação Educar Brasil',         doadorEmail: 'doacoes@fundacaoeducar.org',     status: DonationStatus.CONFIRMED, campaignId: campaigns[1].id, mensagem: 'Investindo no futuro das crianças.' },
    { tipo: DonationType.MONETARY, valor: 3000,  doadorNome: 'Empresa TechSul Ltda',           doadorEmail: 'rh@techsul.com.br',             status: DonationStatus.CONFIRMED, campaignId: campaigns[1].id },
    { tipo: DonationType.MONETARY, valor: 12000, doadorNome: 'Instituto Saúde e Vida',         doadorEmail: 'contato@saudeevida.org',         status: DonationStatus.CONFIRMED, campaignId: campaigns[2].id, mensagem: 'Saúde é direito de todos.' },
    { tipo: DonationType.MONETARY, valor: 5000,  doadorNome: 'Dr. Renato Augusto',             doadorEmail: 'drrenato@clinicaaugusta.com',    status: DonationStatus.CONFIRMED, campaignId: campaigns[2].id },
    { tipo: DonationType.MEDICINE, descricao: 'Kit de medicamentos básicos — 200 unidades (validade 2026)', doadorNome: 'Farmácia Popular do Bairro', status: DonationStatus.CONFIRMED, campaignId: campaigns[2].id },
    { tipo: DonationType.EQUIPMENT,descricao: '2 aparelhos de pressão, 3 glicosímetros, 1 oxímetro',        doadorNome: 'Clínica Saúde Total',         status: DonationStatus.CONFIRMED, campaignId: campaigns[2].id },
    { tipo: DonationType.MONETARY, valor: 10000, doadorNome: 'Construtora Porto Seguro',       doadorEmail: 'obras@portoseguro.com',          status: DonationStatus.CONFIRMED, campaignId: campaigns[3].id },
    { tipo: DonationType.MONETARY, valor: 3500,  doadorNome: 'Loja de Materiais Construção Já',doadorEmail: 'vendas@construcaoja.com',         status: DonationStatus.CONFIRMED, campaignId: campaigns[3].id },
    { tipo: DonationType.CLOTHING, descricao: '500 peças de roupas e calçados em bom estado', doadorNome: 'Bazar Solidário das Damas',                                       status: DonationStatus.CONFIRMED, campaignId: campaigns[6].id },
    { tipo: DonationType.CLOTHING, descricao: '200 peças infantis (0-12 anos)',                doadorNome: 'Escola de Moda Criativa',                                          status: DonationStatus.CONFIRMED, campaignId: campaigns[6].id },
    { tipo: DonationType.MONETARY, valor: 500,   doadorNome: 'Pedro Henrique Silva',           doadorEmail: 'ph.silva@email.com',             status: DonationStatus.CONFIRMED },
    { tipo: DonationType.MONETARY, valor: 1500,  doadorNome: 'Empresa Logística Express',      doadorEmail: 'social@logexpress.com',          status: DonationStatus.PENDING,   campaignId: campaigns[4].id },
    { tipo: DonationType.MONETARY, valor: 800,   doadorNome: 'Maria das Graças Santos',        doadorEmail: 'mgsantos@email.com',             status: DonationStatus.CONFIRMED, campaignId: campaigns[5].id },
  ]
  ...
  // ══════════════════════════════════════════════════════════
  const certData = [
    { vi: 0, tipo: CertificateType.HOURS,        titulo: 'Certificado de Horas Voluntariadas — 2024',      descricao: 'Contribuiu com 96 horas de atendimento médico voluntário ao longo do ano de 2024.',      horasCertificadas: 96,  campaignId: campaigns[2].id },
    { vi: 3, tipo: CertificateType.ACHIEVEMENT,  titulo: 'Voluntária Destaque do Ano — 2024',               descricao: 'Reconhecimento especial pelo impacto excepcional em saúde mental comunitária.',            horasCertificadas: 120, campaignId: campaigns[2].id },
    { vi: 1, tipo: CertificateType.PARTICIPATION,titulo: 'Certificado de Participação — Escola do Futuro', descricao: 'Participou como educadora voluntária no projeto Escola do Futuro, ministrando aulas de reforço escolar.', horasCertificadas: 68, campaignId: campaigns[1].id },
    { vi: 8, tipo: CertificateType.HOURS,        titulo: 'Certificado de Horas Voluntariadas — Saúde',     descricao: 'Dedicou 84 horas de enfermagem voluntária nas feiras de saúde comunitárias.',              horasCertificadas: 84,  campaignId: campaigns[2].id },
    { vi: 9, tipo: CertificateType.PARTICIPATION,titulo: 'Certificado — Apoio Social Comunitário',          descricao: 'Atuou como assistente social voluntária, realizando mais de 200 atendimentos familiares.',  horasCertificadas: 72 },
    { vi: 4, tipo: CertificateType.RECOGNITION,  titulo: 'Certificado de Reconhecimento — Cozinha Solidária', descricao: 'Coordenou a cozinha solidária, preparando refeições para mais de 300 famílias.',       horasCertificadas: 52,  campaignId: campaigns[0].id },
    { vi: 2, tipo: CertificateType.PARTICIPATION,titulo: 'Certificado — Mutirão Moradia Digna',             descricao: 'Participou como engenheiro voluntário na reforma de 5 residências em situação precária.',  horasCertificadas: 44,  campaignId: campaigns[3].id },
  ]
  for (const c of certData) {
    if (!volunteers[c.vi]) continue
  ...
  for (const p of payables) {
    await (prisma as any).payable.create({ data: { ...p, organizationId: org.id } }).catch(() => {})
  }
  ...
  for (const r of receivables) {
    await (prisma as any).receivable.create({ data: { ...r, organizationId: org.id } }).catch(() => {})
  }
